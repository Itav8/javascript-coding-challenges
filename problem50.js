...
Similarly, the numerical value of a string str consisting of some lowercase English letters is the concatenation (not sum!) of the numeric values of each letter in str, which is then converted into an integer.
For example, if str = 'acb', we concatenate each letter's numeric value, resulting in 021 which is then converted to integer 21.
...
const isSumEqual = (firstWord, secondWord, targetWord) => {
  ...
  for (const char of firstWord) {
    first += alphabet.indexOf(char);
  }
  for (const char of secondWord) {
    second += alphabet.indexOf(char);
  }
  for (const char of targetWord) {
    target += alphabet.indexOf(char);
  }
